...
bool UiArm::init()
{
   ...
   // m_sliderNo=true;
    //界面；
    m_pNode = GUIReader::getInstance()->widgetFromJsonFile("uiArm.ExportJson");
    m_pNodeBase->addChild(m_pNode);
    m_pNode->setTouchEnabled(false);
    
    //
    m_pNodeBase->getChildByName("buttonClose")->setVisible(false);
   // Helper::seekWidgetByName(m_pNodeBase,"Image_2")->setVisible(false);
    

    //一个账号里的英雄；
   
    m_equip=Account::sharedAccount()->m_equip;
    m_equipProperty=Account::sharedAccount()->m_equipProperty;
    
    
    //滑块；
    m_slider=static_cast<Slider*>(Helper::seekWidgetByName(m_pNode,"slider"));
    m_slider->setPercent(14);
    m_slider->addEventListener(CC_CALLBACK_2(UiArm::sliderEvent, this));      //事件监听；
    ...
    m_scrollView->addEventListener(  CC_CALLBACK_2(UiArm::scrollViewEvent,this)  );    //事件监听；
...
bool  UiArm::scrollViewInit(){                    //============初始化滚动图层；并添加精灵项；
    ...
    //设置滚动层
    int line=(int)(m_equip.size()/2);
    if(m_equip.size()%2==1){ line+=1; }
    if (line<5) {                                            //比显示窗小时；
        line=5;
    ...
    for(int  i=0;  i<m_equip.size();i++){
        ...
        panelSet->addChild(panelItem2,0,m_equip[i]);         //以ID号为名字；
        
        setItem(panelItem2, m_equip[i]);                    //-------------设置精灵项；
    }
...
void  UiArm::setItem(Widget* imageItem ,string  ID){              //设置一个英雄框；
...
void  UiArm::setHeadBK(Widget* pWidget,string ID){        //设置底框；tag 100, 101
    ...
    //buttonHead->addTouchEventListener(CC_CALLBACK_2(UiArm::buttonEvent, this));
    ...
    buttonImage->addTouchEventListener(CC_CALLBACK_2(UiArm::buttonEvent, this));
    
}


void  UiArm::setHead(Widget* pWidget, string ID){       //设置头像；
    string  str1;
    str1="equip/"+m_equipProperty[atoi(ID.c_str())].table.equip_icon;
...
//void  UiArm::setStar(Widget * pWidget,int starLevel){      //====设置星级；
//    
//
//    
//    Button  *buttonHead=static_cast<Button*>(Helper::seekWidgetByName(pWidget,"buttonHead"));
...
void  UiArm::setLevel(Widget * pWidget,int  level){       //====设置等级；
    
    Text  *textLeve2=static_cast<Text*>(Helper::seekWidgetByName(pWidget,"text3"));
    string  str2=to_string(level)+ "级";
    textLeve2->setString(str2);
    
}

void  UiArm::setHeroName(Widget * pWidget, string  ID, int  quality){       //设置名字；
    
    string name=m_equipProperty[(atoi(ID.c_str()))].table.equip_name;
...
void  UiArm::setText2(Widget * pWidget,  string  ID){       //=====物攻；；
    string  str="物攻：350";
    
    Text  *textName=static_cast<Text*>(Helper::seekWidgetByName(pWidget,"text2"));
    textName->setString(str);
...
void  UiArm::sliderEvent(Ref *pSender, Slider::EventType type){    //===滑块响应函数，控制滚动层；
...
void  UiArm::scrollViewEvent(Ref *pSender, ui::ScrollView::EventType type){   //====滚动层响应函数，控制滑块；；
...
bool  UiArm::buttonInit(){
    ...
    buttonBack->addTouchEventListener(CC_CALLBACK_2(UiArm::buttonEvent, this));
...
void  UiArm::buttonEvent(Ref *pSender, Widget::TouchEventType type)             //=====按钮响应函数；
{
