/*************************
生成MD5码的类；
 MD5码是把一个字符串经过加密转换，变成一个看不懂的字符串；
 作用：输入一个密码，生成MD5码，存放在数据库，别人看到了数据库也解不出密码；
一个字符串总是生成一个相同的MD5码，只是验证的作用，没有所谓的解密；
 
用对象 直接调用 MD5Made(char*  inchar, char* outchar )；
 
 //#include<iostream>  //包含文件放在头文件里出错，放在源文件里不出错；
 //#include<string>
 //#include "MD5.h"
 
 MD5 m5；
 char * ch1="rrefdererrefd43fdfggfgf";
 char  *ch2=new char();
 m5.MD5Made(ch1, ch2);
...
void MD5::MD5Made(char*  inchar, char* outchar ){      // 以字符串的形式输出MD5码，（输入的字符串，输出的字符串）
    ...
    //输入的字符串转换；
    unsigned char *encrypt=(unsigned char*)inchar;
    ...
    char  str[32];
    for (int i=0; i<16; i++) {
        sprintf(str+i*2, "%02X",decrypt[i]);   //(存放的指针，类型，被复制的)
    }
    
    sprintf(outchar,"%s",str);
